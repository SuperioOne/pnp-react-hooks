...
function writeRollupError(err) {
  ...
  if (typeof err.loc === "object" && err.loc.file) {
    console.error(
      `at ${err.loc.file}${err.loc.line > -1 ? `:${err.loc.line}` : ""}${err.loc.column > -1 ? `:${err.loc.column}` : ""}`,
    );
  }
}
...
async function startBuild(opt) {
  ...
  if (opt.visualize) {
    currentConfig.forEach((cfg) => {
      const outName =
        `./.temp/stats_${opt.config}_${cfg.stageName.replace(/[?\\\/:"|*<>()+=~#!@\'\:\;%\^\&\{\}\[\]\s]+/g, "_")}.html`.toLowerCase();

      if (cfg.rollupConfig.plugins) {
        cfg.rollupConfig.plugins.push(
          visualizer({
            filename: outName,
          }),
        );
      } else {
        cfg.rollupConfig.plugins = [
          visualizer({
            filename: outName,
          }),
        ];
...
args
  ...
  .option(
    "visualize",
    "create a visualization graph file for generated code.",
    false,
  )

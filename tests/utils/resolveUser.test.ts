...
test("resolveUser by numeric Id", () => {
  expect(() =>
    resolveUser(Web("http://test.com").siteUsers, 123),
  ).not.toThrow();
});

test("resolveUser by email", () => {
  expect(() =>
    resolveUser(Web("http://test.com").siteUsers, "test@test.com"),
  ).not.toThrow();
});

test("resolveUser by login name", () => {
  expect(() =>
    resolveUser(Web("http://test.com").siteUsers, "i:0#.s|test@test.com"),
  ).not.toThrow();
});
...
test("resolveUser by whitespace only name", () => {
  expect(() =>
    resolveUser(Web("http://test.com").siteUsers, "      "),
  ).toThrow();
});

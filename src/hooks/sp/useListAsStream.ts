import { ContextOptions, ErrorOptions, RenderOptions, BehaviourOptions } from "../../types/options";
import { DisableOptionValueType } from "../../types/options/RenderOptions";
import { IList, IRenderListDataAsStreamResult, IRenderListDataParameters } from "@pnp/sp/lists/types";
...
export interface ListAsStreamOptions extends ErrorOptions, RenderOptions, ContextOptions, BehaviourOptions
{
    disabled?: DisableOptionValueType | { (list: string, parameters: RenderListParameters): boolean; };
}
...
export function useListAsStream(
    ...
    deps?: React.DependencyList): Nullable<IRenderListDataAsStreamResult>
{
    const globalOptions = useContext(InternalContext);
    const [listData, setListData] = useState<Nullable<IRenderListDataAsStreamResult>>();
    ...
    const invokableFactory = useCallback(async (sp: SPFI) =>
    {
        ...
        {
            return this.renderListDataAsStream(_params.dataParameters, overrideParams, queryParams);
        };
    ...
    const _options = useMemo(() =>
    {
        ...
        opt.disabled = checkDisable(opt?.disabled, defaultCheckDisable, list, _params);
...
function _deepCompareParameters(right: RenderListParameters, left: RenderListParameters)
{
    if (right === left)
    {
        return true;
    ...
    {
        return shallowEqual(right.dataParameters, left.dataParameters)

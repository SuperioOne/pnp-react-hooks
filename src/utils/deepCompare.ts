...
import { compareArray, shallowEqual } from "./compare";
import { isNull } from "./is";
import { _PnpHookOptions } from "../types/options";

export function deepCompareOptions(
  left: Nullable<_PnpHookOptions>,
  right: Nullable<_PnpHookOptions>,
) {
  return (
    left === right ||
...
export function deepCompareQuery<
  T extends ODataQueryable | ODataQueryableCollection,
>(left: Nullable<T>, right: Nullable<T>) {
  return (
    left === right ||
    (!isNull(left) && !isNull(right) && deepCompareODataQueryable(left, right))
  );
}

type _ODataQuery = ODataQueryable & ODataQueryableCollection;

export function deepCompareODataQueryable(
  left: _ODataQuery,
  right: _ODataQuery,
) {
  return (
